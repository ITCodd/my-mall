    ...
    public static List<UserTask> iteratorFindParentUserTasks(FlowElement source, Set<String> hasSequenceFlow, List<UserTask> userTaskList) {
        ...
        if (sequenceFlows != null) {
            ...
            for (SequenceFlow sequenceFlow: sequenceFlows) {
                String line=sequenceFlow.getId();
                if(StringUtils.isBlank(line)){
                    line=sequenceFlow.getSourceFlowElement().getId()+"-"+sequenceFlow.getTargetFlowElement().getId();
                }
                // 如果发现连线重复，说明循环了，跳过这个循环
                if (hasSequenceFlow.contains(line)) {
                    continue;
                }
                // 添加已经走过的连线
                hasSequenceFlow.add(line);
